...
def test_training_set(
    ...
    *args,
    ...
    assert (
        X_train.shape[0] == 60
    ), f'Training set for training model should have 60 examples, but has {X_train.shape[0]}'
    assert (
        X_train.shape[1] == 14
    ), f'Training set for training model should have 14 features, but has {X_train.shape[1]}'
    assert (
...
def test_validation_set(
    ...
    *args,
    ...
    assert (
        X_val.shape[0] == 3316156
    ), f'Training set for validation should have 3316156 examples, but has {X_val.shape[0]}'
    assert (
        X_val.shape[1] == 14 
    ), f'Training set for validation should have 14 features, but has {X_val.shape[1]}'
    assert (
